    ...
    public func registerByEmail(authData: AuthRequest?, email: String, password: String, _ context: String? = nil, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        ...
        post("/api/v2/register/email", body) { code, message, data in
            if authData == nil {
                ...
                self.createUserInfo(code, message, data) { code, _, userInfo in
                    if code == 200 {
...
//    }
    ...
    public func getSecurityCaptcha(completion: @escaping (Int, String?, Data?) -> Void) {
        getConfig { config in
            if config != nil {
                let urlString = "\(Authing.getSchema())://\(Util.getHost(config!))/api/v2/security/captcha"
    ...
    public func logout(_ onlySelf: String = "", completion: @escaping (Int, String?) -> Void) {
        get("/api/v2/logout?app_id=\(Authing.getAppId())&only_self=\(onlySelf)") { code, message, _ in
    ...
    public func sendSms(phone: String, phoneCountryCode: String? = nil, completion: @escaping (Int, String?) -> Void) {
        ...
        post("/api/v2/sms/send", body) { code, message, _ in
            completion(code, message)
        }
    }
    ...
    public func sendEmail(email: String, scene: String, completion: @escaping (Int, String?) -> Void) {
        ...
        post("/api/v2/email/send", body) { code, message, _ in
            completion(code, message)
        }
    }
    ...
    public func sendLoginEmail(email: String, scene: String, completion: @escaping (Int, String?) -> Void) {
        let body: NSDictionary = ["email": email, "scene": scene]
        post("/api/v2/email/send-email", body) { code, message, _ in
            completion(code, message)
        }
    }
    ...
    public func deleteAccount(completion: @escaping (Int, String?) -> Void) {
        delete("/api/v2/users/delete") { code, message, _ in
            if code == 200 {
                Authing.saveUser(nil)
                HTTPCookieStorage.shared.cookies?.forEach(HTTPCookieStorage.shared.deleteCookie)
            }
...
//        }
        ...
        getConfig { config in
            ...
            let url = "/connection/social/apple/\(userPoolId!)/callback?app_id=\(Authing.getAppId())"
    ...
    public func loginByGitee(_ code: String, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        getConfig { config in
            guard let conf = config else {
                completion(ErrorCode.config.rawValue, ErrorCode.config.errorMessage(), nil)
                return
            ...
            guard let conId = conf.getConnectionId(type: "gitee:mobile") else {
                completion(ErrorCode.config.rawValue, ErrorCode.config.errorMessage(), nil)
                return
            }
    ...
    public func loginBySlack(_ code: String, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        getConfig { config in
            guard let conf = config else {
                completion(ErrorCode.config.rawValue, ErrorCode.config.errorMessage(), nil)
                return
            ...
            guard let conId = conf.getConnectionId(type: "slack:mobile") else {
                completion(ErrorCode.config.rawValue, ErrorCode.config.errorMessage(), nil)
                return
            }

            let body: NSDictionary = ["connId": conId, "code": code]
            self.post("/api/v2/ecConn/slack/authByCode", body) { code, message, data in
                self.createUserInfo(code, message, data, completion: completion)
            }
        }
    }
    ...
    public func loginByLine(_ accessToken: String, _ idToken: String? = nil, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        getConfig { config in
            ...
            guard let conId = conf.getConnectionId(type: "line:mobile") else {
                completion(ErrorCode.config.rawValue, ErrorCode.config.errorMessage(), nil)
                return
            }
    ...
    public func getDataByWechatlogin(authData: AuthRequest? = nil, code: String, _ context: String? = nil, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        getConfig { config in
            ...
            self.post("/api/v2/ecConn/wechatMobile/authByCodeIdentity", body) { code, message, data in
                if code == 200 {
                    self.createUserInfo(code, message, data) { _, _, userInfo in
        ...
        post("/api/v2/ecConn/wechatMobile/register", body) { code, message, data in
            if code == 200 {
                if let jsonData = data {
                    self.createUserInfo(code, message, jsonData) { _, _, userInfo in
                        self.getCurrentUser(user: userInfo, completion: completion)
                    }
                } else {
    ...
    public func bindWechatByPhoneCode(phoneCountryCode: String? = nil, phone: String, code: String, key: String, completion: @escaping (Int, String?, UserInfo?) -> Void) {
        ...
        post("/api/v2/ecConn/wechatMobile/byPhoneCode", body) { code, message, data in
            if code == 200 {
                if let jsonData = data {
                    self.createUserInfo(code, message, jsonData) { _, _, userInfo in
                        self.getCurrentUser(user: userInfo, completion: completion)
    ...
    // MARK: - --------- Device Manager APIs ----------

    public func createDevice(_ customData: String? = nil, completion: @escaping (Int, String?, NSDictionary?) -> Void) {
        let osName: String
        ...
        post("/api/v3/create-device", body, completion: completion)
    }
